...
| Código | Descrição                                                                                                       |
| ------ | --------------------------------------------------------------------------------------------------------------- |
| RNF01  | O sistema deve responder a ações do usuário em até 2 segundos, em rede estável (ping ≤ 200ms, banda ≥ 10 Mbps). |
| RNF02  | A geração de relatórios em PDF deve ser concluída em no máximo 5 segundos.                                      |
...
| RNF05  | O sistema deve registrar logs com: ID do usuário (ou anônimo), IP, endpoint, data/hora, tipo de ação e status.  |
| RNF07  | Logs devem ser armazenados no MongoDB com retenção mínima de 6 meses.                                           |
| RNF08  | Toda comunicação com APIs externas deve ocorrer via HTTPS.                                                      |
| RNF09  | Toda a interface textual deve estar em português correto, sem erros gramaticais ou termos técnicos de TI.       |
