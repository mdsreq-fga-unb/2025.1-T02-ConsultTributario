<div align="center">
  <h1>🤝 Reunião de Planning </h1>
</div>

## Detalhes da Reunião:

| Tipo                     | Descrição                 |
| ------------------------ | ------------------------- |
| Data da Reunião          | 03/06/2025                |
| Horário da Reunião       | 21:00                     |
| Modalidade               | Videoconferência (Online) |
| Participantes            | Desenvolvedores           |
| Membros Presentes - Devs | Todos estavam presentes   |

### Objetivo da Reunião

Definir o Sprint Goal e atribuir as tarefas prioritárias.

**Sprint Goal:** Disponibilizar **CRUD de Perguntas** com regras de associação validadas.

## Pautas da Reunião

| #   | Pauta                            |
| --- | -------------------------------- |
| 1   | Revisão de backlog & prioridades |
| 2   | Seleção PBIs                     |
| 3   | Planning Poker                   |
| 4   | Riscos & dependências            |
| 5   | Atribuição de atividades         |

## Sprint Backlog Selecionado

| História                                                         | Título                                     | SP (Planning Poker) |
| ---------------------------------------------------------------- | ------------------------------------------ | ------------------- |
| [US‑08](../../features/gerenciar-perguntas-tributarias/us-08.md) | Criar pergunta jurídica                    | 5                   |
| [US‑09](features/gerenciar-perguntas-tributarias/us-09.md)       | Editar pergunta jurídica                   | 3                   |
| [US‑11](features/gerenciar-perguntas-tributarias/us-11.md)       | Visualizar perguntas jurídicas cadastradas | 2                   |

| Tarefas                                 | Descrição                                                                               | Responsável       |
| --------------------------------------- | --------------------------------------------------------------------------------------- | ----------------- |
| Criar uma feature de autenticação (PBB) | Existem dois cargos (admin e user), mas nenhuma documentação a respeito de autenticação | Artur Krauspenhar |

## Riscos & Dependências

| Risco                               | Mitigação                    |
| ----------------------------------- | ---------------------------- |
| Regra de associação entre perguntas | Revisão PO + revisão Cliente |

## Gravação da Reunião:

_Reunião não Gravada_
